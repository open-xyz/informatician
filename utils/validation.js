...
export const AuthErrorMessage = ({ message }) => {
  return (
    <p className="block text-start !text-red-600 text-sm w-[200px] m-0 mb-2">
      {message}
    </p>
  );
};
...
export const validate = {
  ...
  message: (value) => {
    ...
    if (numOfWords < 10 || numOfWords > 100) {
      return {
        message: true,
        messageError: "Describe your message in 10 to 100 words",
      };
    } else {
  ...
  fName: (value) => {
    return value.trim().length < 3
      ? {
          fName: true,
          fNameError: "First Name must be atleast 3 characters long.",
        }
  ...
  lName: (value) => {
    return value.trim().length < 3
      ? {
          lName: true,
          lNameError: "Last Name must be atleast 3 characters long.",
        }
  ...
  bookName: (value) => {
    ...
    if (numOfWords > 10) {
      return {
        bookName: true,
        bookNameError: "Book title can contain maximum 10 words",
      };
    } else {
  ...
  authorName: (value) => {
    ...
    if (numOfWords < 2)
      return {
        authorName: true,
        authorNameError: "Add First Name and Last Name",
      };
    if (value.length > 40)
      return {
        authorName: true,
        authorNameError: "Name must be less than 40 characters",
      };
  ...
  publisher: (value) => {
    if (value.length > 30)
      return {
        publisher: true,
        publisherError: "Publisher name must be less than 30 characterss",
      };
  ...
  pages: (value) => {
    if (value.length < 1)
      return { pages: true, pagesError: "Pages cannot be in negative number" };
    if (value.trim().length < 1)
      return { pages: true, pagesError: "Pages cannot be in negative number" };
    if (value > 100000)
      return { pages: true, pagesError: "Pages must be less than 100000" };
